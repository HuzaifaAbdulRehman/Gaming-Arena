...
// Mode functions i.e singleplayer and multiplayer for hand cricket
void singleplayer()
{
	srand(time(NULL));
...
void multiplayer()//For hand cricket
{
	int no_players, i;
	do
	{
	...
	for (i = 0; i < no_players; i++)
	{
		if (i == no_players - 1)
		{
			int max_i = max(arr, no_players);
			arr[i] = batting(arr[max_i]);
...
//Snake water gun functions i.e multiplayer and singleplayer
void swg_m()
{
